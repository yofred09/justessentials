...
final class TeleportScheduler {
    ...
    static void schedule(ServerPlayer player, Runnable action) {
        ...
        player.sendSystemMessage(Messages.message("&eTeleporting in "+seconds+" seconds. Do not move."));
    }
    static void tick(ServerPlayer player) {
        ...
        if(!p.dimension().equals(player.level().dimension().location().toString())||player.distanceToSqr(p.x(),p.y(),p.z())>tolerance*tolerance){PENDING.remove(player.getUUID());player.sendSystemMessage(Messages.message("&cTeleport cancelled because you moved."));return;}
