...
final class Messages {
        ...
        for (int index = 0; index < input.length(); index++) {
            if (input.charAt(index) == '&' && index + 1 < input.length()) {
                if (input.charAt(index + 1) == '#' && index + 7 < input.length()) {
                    try {
                        int rgb = Integer.parseInt(input.substring(index + 2, index + 8), 16);
                        if (!text.isEmpty()) root.append(styled(text.toString(), color, bold, italic, underlined, strike));
                        text.setLength(0); color = TextColor.fromRgb(rgb); index += 7; continue;
                ...
                if (format != null) {
                    ...
                    if (format == ChatFormatting.RESET) { color = TextColor.fromLegacyFormat(ChatFormatting.WHITE); bold = italic = underlined = strike = false; }
                    else if (format.isColor()) color = TextColor.fromLegacyFormat(format);
    ...
    private static MutableComponent styled(String text, TextColor color, boolean bold, boolean italic, boolean underlined, boolean strike) { return Component.literal(text).withStyle(style -> style.withColor(color).withBold(bold).withItalic(italic).withUnderlined(underlined).withStrikethrough(strike)); }
